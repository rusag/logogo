...
industry_list = sorted(["Accounting",
                        "Airlines / Aviation",
                        ...
                        "Wireless",
                        "Writing / Editing"])
logostyle_list = ['A','B','C']
font_list = ["SpaceGrotesk-Bold, Space Grotesk", "Akira Expanded", "SweetSnow", "HKGrotesk"]
tag_list = ['Cool', 'Sexy', 'Oriental']
symbol_list = ['A', 'B', 'C']
color_group_list = [
  {
    "name": "Absolute Zero",
...
# svg_templates = {'icon typo 2 line +slogan':'template1',
#                 'Icon + Typo Long':'template2',
#                 'Large Icon + Typo 2 line + Slogan':'template3',
#                 'Icon + Typo short + Slogan':'template4',
#                 'Icon + Typo short':'template5',
#                 'Icon + Typo Long тАУ 1':'template6',
#                 'Icon + Typo Long + Slogan':'template7',}

svg_templates = {'template1':'icon typo 2 line +slogan',
                'template2':'Icon + Typo Long',
                'template3':'Large Icon + Typo 2 line + Slogan',
                'template4':'Icon + Typo short + Slogan',
                'template5':'Icon + Typo short',
                'template6':'Icon + Typo Long тАУ 1',
                'template7':'Icon + Typo Long + Slogan',}

# model named Post 
class SVG_POST(models.Model): 
    industry_choices = ((i, i) for i in industry_list)
    logostyle_choices = ((i, i) for i in logostyle_list)
    font_choices = ((i, i) for i in font_list)
    tag_choices = ((i, i) for i in tag_list)
    symbol_choices = ((i, i) for i in symbol_list)
    # color_group_choices = tuple(item for item in list(color_group_list.items()))
    color_group_choices = tuple((item['hex'],item['name']) for item in color_group_list)
    template_choices = tuple((item,svg_templates[item]) for item in svg_templates)
    # define a username filed with bound  max length it can have 
    brand = models.CharField( max_length = 15, blank = False, null = False)
    slogan = models.CharField( max_length = 15,blank = True, null = True)
    style = models.CharField(max_length = 15, choices = logostyle_choices, default='A')
    industry = models.CharField(max_length = 100, choices = industry_choices, default=industry_list[0])
    font = models.CharField(max_length = 100, choices = font_choices, default=font_list[0])
    tag = models.CharField(max_length = 100, choices = tag_choices, default=tag_list[0])
    symbol = models.CharField(max_length = 100, choices = symbol_choices, default=symbol_list[0])
    color = models.CharField(max_length = 15, choices = color_group_choices, default=color_group_list[0]['hex'])
    template = models.CharField(max_length = 15, choices = template_choices, default='template2')


class SVG_FILE(models.Model):
